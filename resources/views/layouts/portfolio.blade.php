...
<body class="antialiased selection:bg-brand/10 bg-background text-text transition-colors duration-300">
    @if(request()->is('/') || request()->is('portfolio'))
        {{-- React Root for Homepage --}}
            ...
            <nav class="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-md border-b border-border px-6 py-4">
                <div class="max-w-7xl mx-auto flex justify-between items-center">
                    ...
                    <div class="hidden md:flex items-center gap-8">
                        <a href="{{ url('/#dossier') }}"
                            class="text-[14px] font-bold text-text-muted hover:text-brand transition-colors">Hồ sơ</a>
                        <a href="{{ url('/#stack') }}"
                            class="text-[14px] font-bold text-text-muted hover:text-brand transition-colors">Công nghệ</a>
                        
                        @if(Route::has('portfolio.projects.index'))
                        <a href="{{ route('portfolio.projects.index') }}"
                            class="text-[14px] font-bold {{ request()->routeIs('portfolio.projects.*') ? 'text-brand' : 'text-text-muted' }} hover:text-brand transition-colors">Dự
                            án</a>
                        @endif

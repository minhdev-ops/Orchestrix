...
class AuthController extends Controller
{
    ...
    {
        $user = User::where('email', $email)->where('is_active', 0)->first();
        if ($user) {
            $metadata = $user->metadata ?? [];
            $hash = md5(($metadata['activation_key'] ?? '').($metadata['key_time'] ?? ''));
            Log::info("Verification attempt for {$email}: Expected {$hash}, Got {$key}");
            if ($hash === $key) {
                $user->is_active = true;
    ...
    public function changePass(Request $r)
    {
        ...
        if (Hash::check($r->oldpass, $u->password)) {
            ...
            if ($token) {
                $tokenModel = PassportToken::find($token->id);
                if ($tokenModel) {
                    $tokenModel->revoke();
                }
            }

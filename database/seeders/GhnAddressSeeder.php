    ...
    public function run(): void
    {
        ...
        foreach ($provinces as $p) {
            Province::updateOrCreate(
                ['province_id' => $p['code']],
                ['province_name' => $p['name'], 'code' => substr($p['codename'] ?? '', 0, 20)]
            );

            $districtRes = Http::get("https://provinces.open-api.vn/api/p/{$p['code']}/?depth=2");
            if ($districtRes->successful()) {
                $provinceData = $districtRes->json();
                foreach ($provinceData['districts'] ?? [] as $d) {
                    District::updateOrCreate(
                        ['district_id' => $d['code']],
                        [
                            'district_name' => $d['name'],
                            'province_id' => $p['code'],
                            'code' => substr($d['codename'] ?? (string) $d['code'], 0, 20),
                        ]
                    );

                    $wardRes = Http::get("https://provinces.open-api.vn/api/d/{$d['code']}/?depth=2");
                    if ($wardRes->successful()) {
                        $districtData = $wardRes->json();
                        foreach ($districtData['wards'] ?? [] as $w) {
                            Ward::updateOrCreate(
                                ['ward_code' => (string) $w['code']],
                                [
                                    'ward_name' => $w['name'],
                                    'district_id' => $d['code'],
                                ]
        ...
        $this->command?->info("Seeded {$provinceCount} provinces, {$districtCount} districts, {$wardCount} wards.");
    }

    private function seedFallbackProvinces(): void
    {
        $fallback = [

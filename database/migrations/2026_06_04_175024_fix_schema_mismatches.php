...
return new class extends Migration
{
    ...
    public function down(): void
    {
        ...
        Schema::table('subscriptions', function (Blueprint $table) {
            ...
            foreach ($drop as $col) {
                if (Schema::hasColumn('subscriptions', $col)) $table->dropColumn($col);
            }
        });
        Schema::table('ai_scanning_jobs', function (Blueprint $table) {
            if (Schema::hasColumn('ai_scanning_jobs', 'asset_id')) $table->dropColumn('asset_id');
            if (Schema::hasColumn('ai_scanning_jobs', 'result')) $table->dropColumn('result');
        });
        Schema::table('store_subscriptions', function (Blueprint $table) {
            if (Schema::hasColumn('store_subscriptions', 'amount_paid')) $table->dropColumn('amount_paid');
        });
    }
};
